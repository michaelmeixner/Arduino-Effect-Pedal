...
#define BSIZE 20000 // DUE has 96K bytes of RAM, use 2*8K of them to give about 1/4 sec max delay
int ringBuff[BSIZE];
...
void setup() {
  ...
  analogWriteResolution(12); // override default 8 bit since we'll use DAC0 on due board
  analogWrite(DAC1,0);
  Serial.begin(9600);
}
...
void loop() {
  ...
  while(1) {
    ...
    // for volume control of delayed signals
    oldValue1 /= 3;
    oldValue2 /= 4;
    ...
    currentTap++;
    if(currentTap >= BSIZE)
      currentTap -= BSIZE; // could just write currentTap == 0; here
